...
# My UOW Test 
    class UoWTests(TestCase):
        ...
        def test_uow_add_update(self):
            ...
            with uow:
                print(f"Bookmarks before: {Bookmark.objects.count()}")
                uow.register_new(self.domain_bookmark_1)
                uow.register_new(self.domain_bookmark_2)
                uow.commit()
                print(f"Bookmarks after commit: {Bookmark.objects.count()}")
                self.assertEqual(Bookmark.objects.count(), 2)

            # After exiting the 'with' block, the transaction records will have been rolled back.
            # The count will be 0 as the changes are not committed.
